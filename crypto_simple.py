...
class PK:  # class for public key
    ...
    def encrypt(self, message, coding):         # encrypt message (use func coding(): str -> int)
...
class RSA:
    ...
    def decrypt(self, c, coding):       # decrypt message (use func coding(): int -> str)
        m = pow_mod(c, self.__get_d(), self.__n)
        return coding(m)

    def sign(self, m, coding):  # sign message (use func coding(): str -> int)
...
class Group:                # class for group
    def __init__(self, *argv):  # creates group with PK objects as arguments
        self.__list = []
        self.__list.extend(argv)

    def add(self, *argv):   # add users to group by PK
        for pk in argv:
    ...
    def remove(self, pk):       # remove user by PK
        if pk in self.__list:
            self.__list.remove(pk)

    def get(self):              # returns list of PK (users who is in the group)
    ...
    def encrypt(self, message, coding):     # encrypt message (use func coding(): str -> int)
        ...
        for member in self.__list:
            codes.append(member.encrypt(message, coding))   # gather data for chinese theorem (values)
            modules.append(member.n)                        # gather data for chinese theorem (modules)
        return chin(modules, codes)     # solution of chinese theorem will be encrypted message
